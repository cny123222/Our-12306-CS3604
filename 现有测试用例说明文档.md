...
#### 5. `TopNavigation.test.tsx`  取消所有skipped，success
...
#### 6. `BottomNavigation.test.tsx` success 完全覆盖
- Tests  1 failed (8)
- **作用**: 测试底部导航栏组件
- **测试内容**:
  - 导航项显示
...
#### 7. `PassengerInfoSection.test.tsx`  success 已修复 全覆盖
- Tests  1 failed (21)
- **作用**: 测试乘客信息选择组件
- **测试内容**:
  - 乘客列表显示
...
#### 8. `OrderConfirmationModal.test.tsx` success 已修复，全覆盖
- Tests  1 failed (4)
- **作用**: 测试订单确认弹窗组件
- **测试内容**:
  - 订单信息显示
...
#### 9. `OrderConfirmationModal.test.FIXED.tsx` 该文件已经被删除
- **作用**: 修复版本的订单确认弹窗测试
- **说明**: 可能是之前版本的修复版本

#### 10. `SuccessModal.test.tsx`：   success 未测覆盖度
- **作用**: 测试成功提示弹窗组件
- **测试内容**:
  - 成功消息显示
  - 自动关闭功能
...
#### 11. `SmsVerificationModal.test.tsx`：success 未测覆盖度
- **作用**: 测试短信验证弹窗组件
- **测试内容**:
  - 验证码输入
  - 提交验证
...
#### 12. `RegistrationVerificationModal.test.tsx`：success 未测覆盖度
- **作用**: 测试注册验证弹窗组件
- **测试内容**:
  - 验证码输入
  - 注册验证流程
...
#### 1. `HomePage.ui-elements.test.tsx`： success 全覆盖
-  Tests：1 failed (17)
- **作用**: 测试首页 UI 元素渲染
- **测试内容**:
  - 导航栏显示
...
#### 2. `HomePage.functional.test.tsx` success 全覆盖
- **作用**: 测试首页功能业务逻辑
- **测试内容**:
  - 出发地验证（为空提示）
  - 到达地验证（为空提示）
...
#### 3. `LoginPage.test.tsx`   success 全覆盖
- **作用**: 测试登录页面
- **测试内容**:
  - 页面渲染
  - 登录表单交互
...
#### 4. `TrainListPage.ui-elements.test.tsx`   success 全覆盖
- **作用**: 测试车次列表页 UI 元素
- **测试内容**:
  - 车次列表显示
  - 筛选面板显示
...
#### 5. `TrainListPage.functional.test.tsx`  success 全覆盖
- **作用**: 测试车次列表页功能
- **测试内容**:
  - 车次搜索
  - 筛选功能（车次类型、席别）
...
#### 6. `OrderPage.ui-elements.test.tsx`  success 全覆盖
- Tests 1 failed (15)
- **作用**: 测试订单填写页 UI 元素
- **测试内容**:
  - 车次信息显示
...
#### 7. `OrderPage.functional.test.tsx`  success 全覆盖
- **作用**: 测试订单填写页功能
- **测试内容**:
  - 乘客选择
  - 席别选择
...
#### 8. `PersonalInfoPage.test.tsx`   success 全覆盖
- **作用**: 测试个人信息页面
- **测试内容**:
  - 用户信息显示
  - 信息编辑功能
...
##### 1. `LoginToRegister.cross.spec.tsx` success
- **作用**: 测试从登录页到注册页的导航流程
- **测试内容**:
...
##### 2. `RegisterFormValidation.cross.spec.tsx`  success
- **作用**: 测试注册表单验证流程
- **测试内容**:
  - 用户名验证（长度、格式、唯一性）
  - 密码验证（长度、复杂度）
...
##### 3. `RegisterToVerification.cross.spec.tsx`  success
- **作用**: 测试注册到验证页的流程
- **测试内容**:
  - 完整信息填写后允许提交
  - 信息不完整时阻止提交
...
##### 4. `LoginFlow.cross.spec.tsx`   success
- **作用**: 测试完整登录流程
- **测试内容**:
  - 登录页 → 短信验证 → 首页
  - 登录成功后保存 token
...
##### 5. `RegisterToLogin.e2e.spec.tsx` success
- **作用**: 测试注册到登录的完整端到端流程
- **测试内容**:
...
##### 6. `HomePage.cross.spec.tsx` success
- **作用**: 测试首页跨页导航流程
- **测试内容**:
  - 首页 → 登录页（通过导航栏）
  - 首页 → 注册页（通过导航栏）
...
##### 7. `TrainList.cross.spec.tsx` success
- **作用**: 测试车次列表页跨页流程
- **测试内容**:
  - 车次列表页 → 登录页
  - 车次列表页 → 注册页
...
##### 8. `HomeToTrainList.e2e.spec.tsx`  success
- **作用**: 测试首页到车次列表页的端到端流程
- **测试内容**:
  - 完整流程：首页填写查询表单 → 车次列表页
  - 勾选高铁/动车后查询
...
##### 10. `OrderSubmission.cross.spec.tsx` success
- **作用**: 测试订单提交流程
- **测试内容**:
...
##### 11. `TrainListToOrder.e2e.spec.tsx`   success
...
##### 12. `CompleteBookingFlow.e2e.spec.tsx`   success
- **作用**: 测试完整订票流程端到端
- **测试内容**:
...
##### 13. `PersonalInfoNavigation.cross.spec.tsx`  success
- **作用**: 测试个人信息页导航流程
- **测试内容**:
  - 个人信息页导航入口
  - 页面跳转功能
...
##### 14. `PhoneVerificationFlow.cross.spec.tsx`  success
- **作用**: 测试手机号验证流程
- **测试内容**:
  - 手机号更新流程
  - 短信验证码发送
...
##### 15. `PassengerManagementFlow.cross.spec.tsx`  success
- **作用**: 测试乘客管理流程
- **测试内容**:
  - 添加乘客
  - 编辑乘客信息
...
##### 16. `OrderHistoryFlow.cross.spec.tsx`  success
- **作用**: 测试订单历史查看流程
- **测试内容**:
  - 订单列表显示
  - 订单详情查看
...
##### 17. `PersonalInfoEditing.cross.spec.tsx`   success
- **作用**: 测试个人信息编辑流程
- **测试内容**:
  - 用户信息编辑
  - 信息验证
...
##### 18. `OrderConfirmation.cross.spec.tsx`  success
- **作用**: 测试订单确认流程
- **测试内容**: 订单确认弹窗交互

##### 19. `OrderConfirmSuccess.cross.spec.tsx`  success
- **作用**: 测试订单确认成功流程
- **测试内容**: 订单确认后的成功提示

##### 20. `OrderConfirmWithSeatAllocation.cross.spec.tsx`  success
- **作用**: 测试带座位分配的订单确认流程
- **测试内容**: 座位分配逻辑验证

##### 21. `OrderPageSeatConsistency.cross.spec.tsx`  success
- **作用**: 测试订单页座位一致性
- **测试内容**: 座位状态一致性验证

##### 22. `OrderPageCrossInterval.integration.spec.tsx`  success
- **作用**: 测试订单页跨区间集成测试
- **测试内容**: 跨区间票价计算验证

##### 23. `LoginStateManagement.integration.spec.tsx`  success
- **作用**: 测试登录状态管理集成测试
- **测试内容**: 登录状态在页面间的保持
...
#### 1. `LoginFlow.integration.test.tsx`  success 
- **作用**: 登录流程集成测试
- **测试内容**:
  - 完整的登录流程
  - API 调用验证
...
#### 2. `RegistrationFlow.integration.test.tsx`  检查skipped原因
-  Tests  6 skipped (6)
- **作用**: 注册流程集成测试
- **测试内容**:
  - 完整的注册流程
