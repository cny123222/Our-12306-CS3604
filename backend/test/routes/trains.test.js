...
describe('车次API测试', () => {
  describe('POST /api/trains/search - 搜索车次列表', () => {
    it('应该验证出发地不为空', async () => {
      ...
      expect(response.body.error).toMatch(/请选择出发地/i);
    });
    ...
    it('应该验证到达地不为空', async () => {
      ...
      expect(response.body.error).toMatch(/请选择到达地/i);
    });
    ...
    it('无效的出发地应该返回错误', async () => {
      ...
      expect(response.body.error).toMatch(/无法匹配该出发地/i);
    });
    ...
    it('无效的到达地应该返回错误', async () => {
      ...
      expect(response.body.error).toMatch(/无法匹配该到达地/i);
    });
  ...
  describe('GET /api/trains/available-dates - 获取可选日期', () => {
    ...
    it('不应该包含已过期的日期', async () => {
      ...
      availableDates.forEach(date => {
        ...
        expect(dateObj >= today).toBe(true);
      });
    });
