...
该脚本会：
1. 自动清空现有车次数据（trains, train_stops, train_cars, train_fares, seat_status等表）
2. 从 `requirements/03-车次列表页/车次信息.json` 读取车次数据
3. 插入列车基础信息（停站、车厢配置、票价）
4. 生成从当前日期开始的15天车次数据
5. 为每个车次、每个座位、每个区间生成座位状态记录
6. 座位号格式为 `车厢号-座位序号`（如 `4-01`），确保全局唯一性

**注意事项：**
- 脚本会自动识别当前日期，无需手动配置
- 生成约226万条座位记录（26个车次 × 15天 × 各车厢座位数 × 各区间）
- 执行时间约2-3分钟，请耐心等待
- 数据库文件不应提交到git，由团队成员本地生成
